...
class PureSpectrumController {
  ...
  surveysOld = async (memberId, params) => {
    ...
    if (eligibilities) {
      ...
      const matchingQuestionIds = eligibilities.map(
        (eg) => eg.survey_question_id
      );
      const matchingAnswerIds = eligibilities.map(
        (eg) => eg.survey_answer_precode_id
      );
      ...
      if (matchingAnswerIds.length && matchingQuestionIds.length) {
        ...
        eligibilities.forEach((eg) => {
          queryString[eg.SurveyAnswerPrecode.precode] =
            eg.SurveyAnswerPrecode.option;
        });
