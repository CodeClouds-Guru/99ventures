...
class LucidController {
	...
	generateEntryLink = async (req, res) => {
		...
		try {
			...
			const result = await Survey.surveyAttemptedByUser(member.id, surveyNumber, 1);
			if (result.length) {
			...
			// Check survey groups
			await this.surveyGroupCheckAndSurveyAttemptEntry(
				surveyNumber, 
				survey.original_json.survey_group_ids, 
				member.id
			);
		}
	};
