...
class FileHelper {
  constructor(files, model, req, new_filename) {
    ...
    if (req) {
      ...
      if (req.body.private == 1) this.private = 'private';
    }
    this.files = files;
    this.model = model;
    this.storage_path = '';
  ...
  async upload(metadata) {
    ...
    for (var key of Object.keys(this.files)) {
      ...
      let mime_type = mime.lookup(new_filename);
      if (!mime_type) {
  ...
  async copyObjects(req_type) {
    ...
    const data = await s3
      .listObjects({ Bucket: process.env.S3_BUCKET_NAME, Prefix: this.model })
      .promise();
  ...
  async zipFiles(res) {
    ...
    let object_keys = [
      'CodeClouds/1/file-manager/abc/new abc/folder2/1665990268319grapefruit-slice-332-332.jpg',
      'CodeClouds/1/file-manager/abc/new abc/folder2/1665990379708grapefruit-slice-332-332.jpg',
    ];
