...
router.get('/pure-spectrum/surveys', async (req, res) => {
  ...
  if (eligibilities) {
    ...
    if (matchingAnswerCodes.length && matchingQuestionCodes.length) {
      ...
      var surveyHtml = '';
      for (let survey of surveys) {
        let link = `/pure-spectrum/entry-link?survey_number=${
      ...
      res.send(
        `<div style="width: 100%; display:flex; flex-wrap: wrap" class="survey-container">${surveyHtml}</div>`
      );
    } else {
...
router.get('/schlesigner/surveys', async (req, res) => {
  ...
  if (matchingAnswerCodes.length && matchingQuestionCodes.length) {
    ...
    eligibilities.map((eg) => {
      queryString['Q' + eg.SurveyQuestion.survey_provider_question_id] =
        eg.precode_id;
    });
    const generateQueryString = new URLSearchParams(queryString).toString();
